...
function SacolaModal({ closeModal }) {
  ...
  const getListas = async () => {
    const potionLista = await PotionService.getLista();
    const sacolaLista = await SacolaService.getLista();

    const encontraNome = (id) => {
      const obj = potionLista.find((i) => i.id === id);
      return (obj && obj.nome) ?? "";
    };

    if (Array.isArray(sacolaLista)) {
      const novaLista = sacolaLista.map(({ potionId, quantidade }) => ({
        nome: encontraNome(potionId),
        quantidade,
      }));
